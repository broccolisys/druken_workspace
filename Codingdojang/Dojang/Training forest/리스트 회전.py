...
save_point = ([0 for i in range(len(sentence) - 1)])
for x in range(len(save_point)):
    direction_ = int(sentence[0])
    if direction_ == 0:
        save_point[(direction_ + x)] = sentence[x + 1]
    elif direction_ > 0 or direction_< 0:
        save_point[(direction_+ x) % len(save_point)] = sentence[x + 1]

print(" ".join(save_point))
